import { getApiProvider } from '@/lib/db';
...
export async function searchTMDBMovies(query) {
  ...
  // If set to strict OMDB
  if (provider === 'omdb') {
    return await searchOMDBRealMovies(cleanQuery);
  }

  // Try TMDB API first (for 'auto' or 'tmdb')
  try {
    ...
    if (res.ok) {
      ...
      if (data.results && data.results.length > 0) {
        return data.results.map((m) => ({
          ...formatTMDBMovieSummary(m),
          apiSource: 'TMDB API'
        }));
      }
    }
  } catch (error) {
    console.warn('TMDB API direct fetch timed out or blocked locally.');
  }

  // If provider is strict TMDB and failed
  if (provider === 'tmdb') {
    return [];
  }
...
export async function getTMDBMovieDetails(tmdbId) {
  ...
  try {
    ...
    if (res.ok) {
      const data = await res.json();
      return {
        ...formatTMDBFullDetails(data),
        apiSource: 'TMDB API'
      };
    }
  } catch (error) {
    console.warn('TMDB details fetch failed/blocked.');
  }
...
export async function getTMDBGenres() {
  try {
    ...
    console.warn('TMDB genres fetch timed out, returning standard genres.');
  }
    ...
    try {
      ...
      if (data.Response === 'True' && Array.isArray(data.Search)) {
        return data.Search.map((m) => ({
          ...
          description: `${m.Title} (${m.Year}) - Real movie search result from database.`,
