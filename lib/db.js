...
const INITIAL_DATA = {
  ...
  sections: [
    ...
    { id: 'sec-drama', name: 'Critically Acclaimed Dramas', visible: true }
  ],
  movies: [
    {
      id: 'movie-top-1',
...
// Global in-memory singleton for Next.js hot reloads and serverless environments
function ensureDataLoaded() {
  ...
  try {
    if (fs.existsSync(DATA_FILE)) {
      ...
      globalThis.__memoryStore = JSON.parse(fileContent);
      if (!globalThis.__memoryStore.apiProvider) globalThis.__memoryStore.apiProvider = 'auto';
      if (!globalThis.__memoryStore.topHeroMovieId) globalThis.__memoryStore.topHeroMovieId = 'movie-top-1';
    } else {
      globalThis.__memoryStore = JSON.parse(JSON.stringify(INITIAL_DATA));
      saveStore(globalThis.__memoryStore);
    }
  } catch (err) {
...
function saveStore(data) {
  ...
  try {
    ...
    console.warn('Could not write to disk, using memory persistence:', err.message);
  }
}
...
export function deleteMovie(id) {
  ...
  if (String(store.topHeroMovieId) === String(id)) {
    store.topHeroMovieId = store.movies[0]?.id || 'movie-top-1';
  }
  saveStore(store);
  return store.movies;
}
...
export function getTopHeroMovie() {
  const store = getStore();
  const heroId = store.topHeroMovieId || 'movie-top-1';
  const heroMovie = store.movies.find(m => String(m.id) === String(heroId)) || store.movies[0] || null;
  return heroMovie;
}
