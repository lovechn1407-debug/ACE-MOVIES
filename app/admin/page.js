...
import { 
  Shield, Layers, Film, Plus, Search, Check, Eye, EyeOff, Trash2, Edit, 
  Share2, Sparkles, RefreshCw, Star, Play, Crown, X, Link as LinkIcon 
} from 'lucide-react';
...
export default function AdminPage() {
  ...
  // Movie Form State (Used for both TMDB save & Manual Add/Edit)
  ...
  const fetchAdminData = async () => {
    try {
      setLoading(true);
      const [secRes, movRes, heroRes] = await Promise.all([
        ...
        for (const g of data.genres) {
          ...
          if (!exists) {
            await fetch('/api/sections', {
              method: 'POST',
              headers: { 'Content-Type': 'application/json' },
              body: JSON.stringify({ name: `${g.name} Movies`, visible: false }) // unchecked by default
            });
            addedCount++;
          }
        }
  ...
  // --- TMDB MOVIE SEARCH & SELECTION HANDLERS ---
  ...
  const handleSelectTmdbMovie = async (movieSummary) => {
    try {
      ...
      console.error(err);
      showToast('Failed to fetch full TMDB details');
    } finally {
  ...
  return (
    <div className="min-h-screen flex flex-col bg-[#08080c] text-white">
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 border-b border-white/10 pb-6">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-gray-400">
                Configure sections, upload TMDB / manual movies, customize play links & set Top 1 Netflix Hero movie.
              </p>
            </div>
          </div>
          ...
          {heroMovie && (
            <div className="flex items-center gap-3 bg-white/5 border border-white/10 px-4 py-2 rounded-xl text-xs">
              ...
              <div>
                <span className="text-gray-400 block text-[10px]">CURRENT TOP #1 MOVIE</span>
          ...
          <div className="space-y-6">
            <div className="flex flex-wrap items-center justify-between gap-4 bg-[#12131a] p-5 rounded-2xl border border-white/10">
              <div>
                ...
                <p className="text-xs text-gray-400">
                  Toggle checkboxes to hide or show sections on the public site. Checkbox state saves immediately.
                </p>
              </div>
          ...
          <div className="space-y-6">
            ...
            <div className="flex flex-wrap items-center justify-between gap-4 bg-[#12131a] p-5 rounded-2xl border border-white/10">
              <div>
                ...
                <p className="text-xs text-gray-400">
                  Add movies from TMDB API or manually, configure streaming play links & sections.
                </p>
              </div>
              ...
              <div className="flex flex-wrap items-center gap-3">
                ...
                >
                  ...
                  <span>Add Movie from Database (TMDB)</span>
                </button>
      ...
      {/* --- MODAL 2: TMDB AUTO-SEARCH & CONFIGURE MODAL --- */}
      {showTmdbModal && (
          ...
          <div className="bg-[#12131a] border border-white/15 rounded-2xl max-w-3xl w-full max-h-[90vh] overflow-y-auto p-6 space-y-6">
            ...
            <div className="flex items-center justify-between border-b border-white/10 pb-3">
              <div className="flex items-center gap-2">
                ...
                <h3 className="text-lg font-bold text-white">Add Movie from TMDB Database</h3>
              </div>
            ...
            <form onSubmit={handleTmdbSearch} className="flex gap-3">
              <input
                type="text"
                placeholder="Search TMDB movie name (e.g. Inception, Dune, Avengers)..."
                value={tmdbQuery}
              ...
              <div className="space-y-3">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 max-h-60 overflow-y-auto pr-1">
                  {tmdbResults.map((res) => (
                    ...
                    >
                      {res.poster && <img src={res.poster} alt={res.title} className="w-12 h-16 object-cover rounded-lg flex-none" />}
                      <div className="overflow-hidden">
                        <h5 className="font-bold text-sm text-white truncate">{res.title}</h5>
                        <p className="text-xs text-gray-400">{res.releaseYear} • Rating: {res.rating}</p>
                      </div>
                    </div>
                  ))}
                </div>
            ...
            {fetchingDetails && (
              <div className="py-8 text-center space-y-2">
                ...
                <p className="text-xs text-gray-400">Fetching full movie details from TMDB server proxy...</p>
              </div>
            )}
              ...
              <form onSubmit={handleSaveMovieForm} className="space-y-5 border-t border-white/10 pt-5">
                <div className="flex items-center gap-4 bg-white/5 p-4 rounded-xl border border-white/10">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-gray-400 line-clamp-2 mt-1">{selectedTmdbMovie.description}</p>
                  </div>
                </div>
          ...
          <div className="bg-[#12131a] border border-white/15 rounded-2xl max-w-2xl w-full max-h-[90vh] overflow-y-auto p-6 space-y-5">
            ...
            <form onSubmit={handleSaveMovieForm} className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  <label className="block text-xs font-semibold text-gray-300 mb-1">TMDB Rating</label>
                  <input
                    type="text"
      ...
      {showTop1Modal && (
        <div className="fixed inset-0 z-50 bg-black/85 backdrop-blur-md flex items-center justify-center p-4">
          <div className="bg-[#12131a] border border-white/15 rounded-2xl max-w-lg w-full p-6 space-y-4">
            ...
            <p className="text-xs text-gray-400">
              Select one of the movies from your saved list below to feature as the Top #1 Hero Movie on the Netflix homepage.
            </p>
